...
### Added
- **`footprint_single_component` check** — two closed boxes standing apart pass every other check, since each endpoint meets another wall and each ring closes, while the outline walk silently measures one of them. This is what several views of one building read as several buildings looks like. Adjacency uses the same endpoint-on-span rule as `wall_loop_closure`, so an ordinary interior partition is not flagged, and only groups standing clear of the main building's extents count — a free-standing interior core sits inside them and passes
- **Footprint shape scoring** — the harness could only measure a bounding box, so an L-shaped plan and a plain box with the same extents scored identically and "the model defaulted to a rectangle" was unmeasurable. The `l-shaped-plan` fixture was added to catch exactly that failure and asserted nothing but `floors: 1`. The scorer now walks the outer outline of the ground-floor walls and reports `footprintCorners`, `footprintArea`, and `footprintFill`, which fixtures assert through `footprintShape` with optional `minCorners`, `maxCorners`, `minFill`, and `maxFill`. A rectangle traces 4 corners at fill 1.0; an L traces 6 at about 0.75. Against the existing captures, `l-shaped-plan` reads 12 corners at 0.89 and `cad-plan-clean` reads 4 at 1.0, so both fixtures now score what their notes always claimed
- **`footprint_traceable` check** — ground-floor walls that touch each other but enclose nothing now fail. `wall_loop_closure` can miss this, since every endpoint meets something even when the pieces never form a ring. An outline enclosing less than a square metre counts as untraced, because a walk out and back along two walls returns to its start having enclosed nothing
...
- **Material inference** — elements can now carry a `material` (concrete, wood, steel, glass, brick, stone, drywall, aluminum), read from the image rather than falling back to the type default for everything. This is why a dark tiled roof and a timber-clad gable both rendered in the same stock brown. An unrecognised value is dropped rather than passed on, since `getMaterialForElement` would otherwise resolve it to concrete and disguise a bad value as a deliberate one. Measured drawings are told to send `"unknown"` unless the drawing labels a material. The sentinel is a plain string rather than null because a null inside an enum is a schema construct the API has never been asked to accept here, and a schema it rejects would break every request rather than an edge case
