...
- **`server/generate.ts`** — `runGeneration()`, the single Anthropic call shared by both proxies, plus the `HEARTBEAT_BYTE`/`HEARTBEAT_MS` constants that keep the response alive past the platform's invocation timeout
- **`server/apiProxy.ts`** — Vite dev server plugin that proxies `/api/generate-floor-plan` to Anthropic API (keeps API key server-side); transport only
...
- The `material` property uses the string `"unknown"` rather than null for "use the type default". A null inside an enum, or an enum on a nullable type, is untested against the API here, and a schema it rejects fails every request — so the property sticks to the plain string enum the `type` property already proves works. `readMaterial` maps anything outside the eight known values, `"unknown"` included, back to undefined.
- **The schema is what the model is allowed to build.** It exposes 11 of the app's 21 `BimElementType` values, plus `material` and a roof `rotation`. Anything missing from the enum comes back as the nearest permitted shape rather than as nothing — a balcony as columns, a glazed gable as ordinary windows — which reads as a model failure but is a schema gap. When adding an element type to the app, decide whether the model should be able to emit it, and if so add it here, to `SUPPORTED_TYPES` and the third-pass switch in `validateAndFixElements`, and to `server/prompt.ts`. All four have to move together.
- Roofs are gables only. `buildRoofMesh` extrudes a triangular profile, and `rotation` picks which axis the ridge runs along — the span and extrusion swap on a quarter turn so a rectangular roof still covers its footprint. There is no hip, mono-pitch, or dormer geometry, so the prompt tells the model to approximate a hip with a gable and to model a dormer as a window on the wall below rather than inventing a shape that cannot be built.
...
- Requests are streamed from the API (`.stream().finalMessage()`) because a 32k `max_tokens` on a non-streaming request risks an HTTP timeout. That covers the **upstream** leg only.
- **The response to the browser is also streamed, and it has to be.** Vercel kills an edge function that has not begun responding within about 25 seconds, and a generation at effort `"high"` runs 2-4 minutes. A handler that awaits the whole message before returning never responds at all: the platform substitutes its own plain-text error page, and the client fails to parse it as JSON (observed as ``Unexpected token 'A', "An error o"...``). Both proxies therefore emit a `HEARTBEAT_BYTE` (a single space) immediately and every `HEARTBEAT_MS`, then write the JSON payload. Whitespace is legal JSON ahead of the value, so the body stays a plain JSON document with no envelope to unwrap. The dev proxy has no such timeout but mirrors the behaviour anyway, or a bug in the streamed path would only ever appear in production.
- **Status codes split at the generation call.** Everything cheap — method, body parse, API key, `validateGenerateRequest` — answers with a real status before the response is committed. Past that point the status is necessarily 200, because the platform requires bytes before the outcome is known, so a generation failure is reported as `{ "error": ... }` in a 200 body. `readServerPayload()` in `aiFloorPlanService.ts` is the client-side counterpart: it tolerates the leading whitespace, checks `error` regardless of status, and reports a non-JSON body as a likely timeout rather than as a JSON syntax error.
- `server/generate.ts` holds `runGeneration()`, shared by both proxies so the request they send cannot drift — the same reason the model list and schema live in `aiConfig.ts`. The proxies contain only transport.
- **Cancellation is plumbed end to end** and needs to stay that way: the modal holds an `AbortController`, `generateFloorPlan` passes its signal to `fetch`, both proxies abort when the caller disconnects, and `runGeneration` forwards the signal to the Anthropic SDK. Without it a generation abandoned after ten seconds still runs to completion and bills for it. Every write to a client that may have gone away is guarded — an unguarded `res.write` emits a fatal unhandled `error` in the dev server, and an unguarded `controller.enqueue` rejects inside the stream's `start`.
- `ALLOWED_MEDIA_TYPES` is the single source for accepted image formats, and `ALLOWED_FORMATS_LABEL` derives the UI copy from it. `AiGenerateModal` uses both for its `accept` attribute, its dropzone hint, and its runtime check — do not reintroduce a looser `image/*` test there, or unsupported files (HEIC especially, the iPhone default) are accepted by the UI and rejected minutes later by the server.
...
### Evals
`evals/` scores AI generation against fixture images instead of judging it by eye. Checks run on the **raw model response**, before `validateAndFixElements` repairs anything, so they measure the model rather than the validator. See `evals/README.md` for the check list. A 10-image corpus covering 7 stratified fixtures ships in `evals/fixtures/` (licences in `ATTRIBUTION.md`); `cad-plan-clean` is the control case — if it fails, the prompt or schema is at fault rather than model vision. Captured runs live in `evals/runs/` (gitignored). Footprint **shape** is scored separately from extents: `footprintMeters` is only a bounding box, so the scorer also walks the outer outline of the ground-floor walls and reports `footprintCorners`, `footprintArea`, and `footprintFill`, which a fixture asserts through `footprintShape`. `footprintFill` is measured against the outline's own bounding box, not against every ground wall — otherwise a detached second building or a stray spur inflates the box and a plain rectangle reads as highly articulated, which would satisfy a `maxFill` assertion. `footprint_single_component` catches that case directly, flagging a wall group standing clear of the main building's extents (a free-standing interior core is not flagged, since it sits inside them). A rectangle is 4 corners at fill 1.0 and an L about 6 at 0.75, so "the model defaulted to a box" is now a number rather than an impression. Scoring reads only from disk, so re-score after changing a check instead of spending another capture. Where a drawing genuinely admits more than one storey count, the fixture asserts `floorsRange: [min, max]` rather than an exact `floors` — `hand-sketch` has a roof belvedere served by a stair drawn in the plan, so both 1 and 2 are defensible and an exact assertion would score the ambiguity rather than the model.
